...
TEMPLATE_INPUT_ADD = """
`c`<m|text|Neue Nachricht`>

`[Hinzufügen/Absenden`:`*|action=add]`        `[Aktualisieren`:]`
"""

TEMPLATE_INPUT_EDIT = """
`c`<m|text|Nachricht bearbeiten`{text}>

`[Speichern`:`*|id={id}|edit=True]`        `[Aktualisieren`:]`
"""

# Screen template - Entry (Use for each entry)
TEMPLATE_ENTRY_ADMIN = """>>>{name} `[lxmf@{dest}]
{date_time}
{text}
`[BEARBEITEN`:`*|id={id}]`        `[LÖSCHEN`:`*|id={id}|delete=true]`
"""

TEMPLATE_ENTRY_OWNER = """>>>{name} `[lxmf@{dest}]
{date_time}
{text}
`[BEARBEITEN`:`*|id={id}]`        `[LÖSCHEN`:`*|id={id}|delete=true]`
"""

TEMPLATE_ENTRY_USER = """>>>{name} `[lxmf@{dest}]
{date_time}
{text}
"""
